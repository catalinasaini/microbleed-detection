...
def preprocess_subject(subject): # MODIFIED FUNCTION -> FRST is computed from original non-inpainted image, and vessel inpainting is done after FRST computation
    
    # Load image
    image_path = subject['input_path']
    image = nib.load(image_path).get_fdata()

    # Keep original, non-inpainted image for FRST
    original_image = image.copy()

    # Brain mask should be based on the original image
    brain_mask = (original_image > 0).astype(int)
    ...
    try:
        ...
        label = np.zeros_like(original_image, dtype=int)

    # Load / compute FRST BEFORE vessel inpainting
    # Important: if frst_path does not exist, FRST is computed from the original non-inpainted image.
    try:
        frst_path = subject['frst_path']
        frst = nib.load(frst_path).get_fdata()
    except:
        frst = get_frst_data(original_image)
        frst[np.isnan(frst)] = 0
        frst -= np.min(frst)
        if np.max(frst) != 0:
            frst /= np.max(frst)
    ...
    # Inpaint vessels AFTER FRST has already been created
    image = inpaint_vessels(original_image, brain_mask)
