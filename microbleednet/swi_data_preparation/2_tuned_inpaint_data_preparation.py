...
# 10-01-2023                              #
#                                         #
# Modified by Catalina Saini              #
# 01-06-2026                              #
###########################################

# Frangi parameters for SWI.
# Goal: avoid inpainting true compact CMB-like structures.
FRANGI_SIGMAS = np.arange(1.0, 3.01, 0.5)
FRANGI_ALPHA = 0.5
FRANGI_BETA = 0.5
FRANGI_BLACK_RIDGES = True

# 2D shape criteria for accepting a component as vessel-like.
VESSEL_MIN_AREA = 8
VESSEL_MIN_MAJOR_AXIS_LENGTH = 6
VESSEL_MIN_ECCENTRICITY = 0.95
...
def _safe_minmax_normalize(data, mask=None):
    """
    Min-max normalize data safely, optionally only within a mask.
    Values outside the mask are kept as zero.
    """
    ...
    if mask is None:
        ...
        valid_mask = mask.astype(bool) & np.isfinite(data)
    ...
    if vmax > vmin:
        out[valid_mask] = (data[valid_mask] - vmin) / (vmax - vmin)

    return out


def save_nifti_like(data, reference_img, output_path):
    """
    Save data as a NIfTI file using affine/header from a reference image.
    """
    data_to_save = np.asarray(data, dtype=np.float32)
    out_img = nib.Nifti1Image(data_to_save, reference_img.affine, reference_img.header)
...
def inpaint_vessels(
    ...
    vessel_min_area=VESSEL_MIN_AREA,
    vessel_min_major_axis_length=VESSEL_MIN_MAJOR_AXIS_LENGTH,
    vessel_min_eccentricity=VESSEL_MIN_ECCENTRICITY
):
    """
    Conservatively inpaint vessel-like structures and always return
    intermediate debugging volumes.
    """

    height, width, depth = image.shape

    frangi_output_volume = np.zeros_like(image, dtype=float)
    linearity_volume = np.zeros_like(image, dtype=float)
    raw_kmeans_mask_volume = np.zeros_like(image, dtype=np.uint8)
    shape_filtered_vessel_mask_volume = np.zeros_like(image, dtype=np.uint8)

    for slice_idx in tqdm(range(depth), leave=False, desc='generating_vessel_masks', disable=True):

        image_slice = image[:, :, slice_idx]
        brain_slice_mask = brain_mask[:, :, slice_idx].astype(bool)

        if np.min(image_slice) == np.max(image_slice):
            continue

        if np.sum(brain_slice_mask) < 10:
            continue

        # 1) Frangi output
        frangi_output_slice = frangi(
            image_slice,
        ...
        frangi_output_slice = np.nan_to_num(frangi_output_slice)
        frangi_output_slice = _safe_minmax_normalize(frangi_output_slice, brain_slice_mask)
        frangi_output_volume[:, :, slice_idx] = frangi_output_slice

        # 2) Linearity
        _, _, linearity_slice = eigenvalues_and_linearity_measure(image_slice)
        linearity_slice = np.nan_to_num(linearity_slice)
        linearity_slice = _safe_minmax_normalize(linearity_slice, brain_slice_mask)
        linearity_volume[:, :, slice_idx] = linearity_slice

        # 3) KMeans mask
        slice_features = np.stack(
            [
                frangi_output_slice[brain_slice_mask].ravel(),
                linearity_slice[brain_slice_mask].ravel()
            ],
            axis=1
        )

        if slice_features.shape[0] < 2:
            continue

        if np.unique(slice_features, axis=0).shape[0] < 2:
            continue

        clusterer = KMeans(n_clusters=2, random_state=42, n_init=10).fit(slice_features)

        cluster_scores = clusterer.cluster_centers_[:, 0] + clusterer.cluster_centers_[:, 1]
        vessel_cluster_label = int(np.argmax(cluster_scores))

        raw_kmeans_mask = np.zeros((height, width), dtype=bool)
        raw_kmeans_mask[brain_slice_mask] = clusterer.labels_ == vessel_cluster_label
        raw_kmeans_mask_volume[:, :, slice_idx] = raw_kmeans_mask.astype(np.uint8)

        # 4) Shape-filtered vessel mask
        labelled_mask = label(raw_kmeans_mask)
        mask_props = regionprops(labelled_mask)

        for mask_prop in mask_props:

            keep_as_vessel = (
                mask_prop.area >= vessel_min_area and
                mask_prop.major_axis_length >= vessel_min_major_axis_length and
                mask_prop.eccentricity >= vessel_min_eccentricity
            )

            if not keep_as_vessel:
                labelled_mask[labelled_mask == mask_prop.label] = 0

        shape_filtered_vessel_mask = labelled_mask > 0
        shape_filtered_vessel_mask_volume[:, :, slice_idx] = shape_filtered_vessel_mask.astype(np.uint8)

    final_vessel_mask = (shape_filtered_vessel_mask_volume > 0).astype(float)

    # 5) Inpainted image
    inpainted_volume = inpaint_with_neighborhood_mean(image, final_vessel_mask.copy())

    debug_outputs = {
        "frangi_output": frangi_output_volume,
        "linearity": linearity_volume,
        "raw_kmeans_mask": raw_kmeans_mask_volume.astype(float),
        "shape_filtered_vessel_mask": shape_filtered_vessel_mask_volume.astype(float),
        "inpainted_image": inpainted_volume
    }

    return inpainted_volume, debug_outputs
...
def preprocess_subject(subject):
    ...
    image = input_img.get_fdata()

    # Define automatic debug-output folder
    input_dir = os.path.dirname(image_path)
    debug_output_dir = os.path.join(input_dir, "inpainting_debug")
    os.makedirs(debug_output_dir, exist_ok=True)

    base_name = os.path.basename(image_path)
    base_name = base_name.replace(".nii.gz", "")
    base_name = base_name.replace(".nii", "")

    # Inpaint vessels and always save intermediate outputs
    brain_mask = (image > 0).astype(int)

    image, debug_outputs = inpaint_vessels(
        image,
        brain_mask
    )

    save_nifti_like(
        debug_outputs["frangi_output"],
        input_img,
        os.path.join(debug_output_dir, base_name + "_frangi_output.nii.gz")
    )

    save_nifti_like(
        debug_outputs["linearity"],
        input_img,
        os.path.join(debug_output_dir, base_name + "_linearity.nii.gz")
    )

    save_nifti_like(
        debug_outputs["raw_kmeans_mask"],
        input_img,
        os.path.join(debug_output_dir, base_name + "_raw_kmeans_mask.nii.gz")
    )

    save_nifti_like(
        debug_outputs["shape_filtered_vessel_mask"],
        input_img,
        os.path.join(debug_output_dir, base_name + "_shape_filtered_vessel_mask.nii.gz")
    )

    save_nifti_like(
        debug_outputs["inpainted_image"],
        input_img,
        os.path.join(debug_output_dir, base_name + "_inpainted_image.nii.gz")
    )
