...
def generate_original_inpainting_debug_outputs(image, brain_mask):
    """
    Generate intermediate debug outputs using the ORIGINAL inpaint_vessels()
    logic and parameters, without modifying inpaint_vessels() itself.

    Saves/returns:
    - frangi_output
    - linearity
    - raw_kmeans_mask
    - shape_filtered_vessel_mask
    - inpainted_image
    """

    frangi_output_volume = np.zeros_like(image, dtype=float)
    linearity_volume = np.zeros_like(image, dtype=float)
    raw_kmeans_mask_volume = np.zeros_like(image, dtype=float)
    shape_filtered_vessel_mask_volume = np.zeros_like(image, dtype=float)

    labelled_mask_volume = np.zeros_like(image)

    height, width, depth = image.shape

    for slice_idx in tqdm(range(depth), leave=False, desc='generating_original_debug_masks', disable=True):
        ...
        # ORIGINAL linearity calculation
        _, _, linearity = eigenvalues_and_linearity_measure(slice)
        linearity = linearity * brain_mask[:, :, slice_idx].astype(float)
        linearity -= np.min(linearity)
        linearity = linearity / np.max(linearity) if np.max(linearity) != 0 else linearity
        linearity_volume[:, :, slice_idx] = linearity

        # ORIGINAL KMeans features
        slice_features = np.stack([frangi_output_slice.ravel(), linearity.ravel()], axis=1)

        clusterer = KMeans(n_clusters=2, random_state=42).fit(slice_features)
        clusters = clusterer.predict(slice_features)

        # ORIGINAL assumption: vessel cluster is the smaller cluster
        vessel_cluster_label = 1 if (clusters == 1).sum() < (clusters == 0).sum() else 0

        raw_kmeans_mask = np.reshape(clusters == vessel_cluster_label, (height, width))
        raw_kmeans_mask_volume[:, :, slice_idx] = raw_kmeans_mask.astype(float)

        # ORIGINAL shape filtering
        ...
        shape_filtered_vessel_mask_volume[:, :, slice_idx] = shape_filtered_vessel_mask.astype(float)

        labelled_mask_volume[:, :, slice_idx] = labelled_mask

    labelled_mask_volume = (labelled_mask_volume > 0).astype(float)

    # ORIGINAL inpainting operation, but use a copy because inpaint_with_neighborhood_mean mutates the mask
    inpainted_image = inpaint_with_neighborhood_mean(image, labelled_mask_volume.copy())

    debug_outputs = {
        "frangi_output": frangi_output_volume,
        "linearity": linearity_volume,
        "raw_kmeans_mask": raw_kmeans_mask_volume,
        "shape_filtered_vessel_mask": shape_filtered_vessel_mask_volume,
...
def preprocess_subject(subject):
    ...
    # Create automatic debug-output folder
    input_dir = os.path.dirname(image_path)
    debug_output_dir = os.path.join(input_dir, "inpainting_debug_original")
    ...
    # Generate and save original inpainting debug outputs
    debug_outputs = generate_original_inpainting_debug_outputs(image, brain_mask)
    ...
    # Use the original inpainted image generated by the duplicated original logic
    image = debug_outputs["inpainted_image"]
