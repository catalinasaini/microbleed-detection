...
def derive_inpainted_path(input_path): # ADDED FUNCTION
    """
    Derive default output path for saved inpainted image.
    Example:
    sub-001_ses-01_swi.nii.gz -> sub-001_ses-01_swi_inpainted.nii.gz
    """
    ...
    if basename.endswith(".nii.gz"):
        stem = basename[:-7]
        out_name = stem + "_inpainted.nii.gz"
...
def save_nifti_like(reference_img, data, output_path): # ADDED FUNCTION
    """
    Save data as NIfTI using affine/header from reference image.
    """
...
# def preprocess_subject(subject): # ORIGINAL FUNCTION
...
def preprocess_subject(subject): # MODIFIED FUNCTION -> Continue using the original non-inpainted image; does not replace image with inpainted_image.
    ...
    # Brain mask from original image
    brain_mask = (image > 0).astype(int)

    # Generate inpainted image, but DO NOT use it downstream
    inpainted_image = inpaint_vessels(image, brain_mask)

    # Save inpainted image only for inspection/output
    inpainted_path = subject.get('inpainted_path', derive_inpainted_path(image_path))
    save_nifti_like(input_img, inpainted_image, inpainted_path)
    ...
    # Invert original non-inpainted image
    image = 1 - (image / np.max(image))
    image = image * brain_mask.astype(float)
